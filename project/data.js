/* ============================================================
   ...
   --
   This is mock-but-plausible data: directionally informed by
   public-domain industry knowledge as of early 2026, NOT a
   proprietary dataset. All operator/brand names are illustrative.
   ============================================================ */
...
(function () {
  ...
  function formatINR(rupees, opts = {}) {
  ...
  // -------------------------------------------------------------
  // 3. Hospitality dataset
  //    Pan-India base — keys per submarket
  // -------------------------------------------------------------
  const HOSPITALITY = {
    summary: {
      keysFY26: 178400,           // branded keys
      pipelineKeys: 92800,        // signed, under development
      systemAvgADR: 8950,         // INR
      systemRevPAR: 6470,
      occFY26: 72.3,              // %
      yoyKeysPct: 8.2,
      yoyADRPct: 11.6,
      yoyRevPARPct: 14.2,
      branded_pct_fy26: 38.2,     // share of branded vs unbranded keys
    },
    quarterlyADR: [   // [Q1, Q2, Q3, Q4] FY26
    ...
    submarkets: [
      // city scoped — relative to pan; submarkets are submarkets WITHIN that city
  ...
  const HEALTHCARE = {
    summary: {
      bedsFY26: 218400,             // branded / corporate beds
      pipelineBeds: 84600,
      utilisationFY26: 68.4,        // %
      arpobFY26: 56400,             // INR / bed / day
      yoyBedsPct: 9.4,
      yoyARPOBPct: 8.1,
      yoyUtilPct: 1.6,
    },
  ...
  // -------------------------------------------------------------
  // 5. Industrial RE dataset
  // -------------------------------------------------------------
  const INDUSTRIAL = {
    summary: {
      stockMSF: 412,                 // existing Grade-A logistics + IP stock
      absorptionMSF_FY26: 58.4,
      vacancyFY26: 8.6,             // %
      avgRentSqftMonth: 24.2,       // INR / sqft / month
      yoyAbsorptionPct: 14.4,
      yoyRentPct: 6.8,
    },
    submarkets: [
      // city, name, rent (INR/sqft/mo), vacancy%, stock (mn sqft), absorption FY26 (mn sqft)
  ...
  const COMMERCIAL = {
    summary: {
      ...
      vacancyFY26: 14.8,
      avgRentSqftMonth: 96.0,    // INR / sqft / month — Grade A weighted
  ...
  // -------------------------------------------------------------
  // 7. Residential RE dataset
  // -------------------------------------------------------------
  const RESIDENTIAL = {
    summary: {
      launchesFY26: 528000,        // units launched
      salesFY26: 502000,
      unsoldStock: 938000,
      psfFY26: 9840,               // weighted INR/psf
      yoyLaunchesPct: 6.4,
      yoySalesPct: 8.2,
      yoyPSFPct: 9.6,
  ...
  // -------------------------------------------------------------
  // 10. Land Pitch Builder — sample land parcels (browse-able)
  // -------------------------------------------------------------
  const LAND_PARCELS = [
  ...
  // -------------------------------------------------------------
  // 11. Master / cross-sector summary
  // -------------------------------------------------------------
  const MASTER = {
    sectors: [
  ...
  // -------------------------------------------------------------
  // 12. Helpers exposed globally
  // -------------------------------------------------------------
  // Filter a submarket array down to the selected city. 'pan' returns all.
  function filterByCity(items, cityId) {
    if (!cityId || cityId === 'pan') return items;
    return items.filter(it => it.city === cityId);
  }

  // City multiplier for top-line numbers when city != pan
  function cityMult(cityId) {
    const c = CITIES.find(c => c.id === cityId);
    return c ? c.mult : 1;
  }

  // Source line per sector / table
  const SOURCES = {
    ...
    crm:         'Pipeline data: simulated CRM sync (Salesforce-like) · last refresh: 04 May 2026, 09:14 IST',
  };

  // -------------------------------------------------------------
  // Open questions for Reliant (annex + chat)
  // -------------------------------------------------------------
