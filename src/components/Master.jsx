...
import { Icon, KPI, SectionHead, Source, Tabs, Delta, PipelineFunnel, RankingBars } from './Primitives.jsx';
  ...
  return (
    <Fragment>
      ...
      <Tabs items={[
        ...
        { id:'pipeline', label:'Pipeline', count: MASTER.pipeline_stages.reduce((s,x)=>s+x.count,0) },
        { id:'hot',      label:'Hot deals', count: MASTER.hot_deals.length },
...
function MasterOverview({ market, onJump }) {
  return (
    <Fragment>
      ...
      <div className="grid cols-2" style={{ marginBottom: 32 }}>
        <div className="card">
          ...
          <PipelineFunnel stages={MASTER.pipeline_stages}/>
        ...
        <div className="card">
          ...
          <div className="col" style={{ gap: 0 }}>
            {MASTER.hot_deals.slice(0,5).map((d,i) => (
      ...
      <div className="grid cols-2">
        <div className="card">
          ...
          <RankingBars
            data={CITIES.filter(c => c.id !== 'pan').slice(0,8).map(c => ({
              label: c.name,
  ...
  return (
    <Fragment>
      <div className="grid cols-4" style={{ marginBottom: 24 }}>
        <KPI label="Pipeline value" value={formatCr(MASTER.pipeline_stages.reduce((s,x)=>s+x.value_cr,0))} delta={18.4} sub="vs FY25"/>
        <KPI label="Open deals" value={MASTER.pipeline_stages.slice(0,5).reduce((s,x)=>s+x.count,0)} delta={12.0}/>
        <KPI label="Closed FY26" value={MASTER.pipeline_stages[5].count} delta={21.4}/>
    ...
    <div className="card" style={{ padding: 0 }}>
      <div className="tbl-wrap" style={{ border: 0, maxHeight: 'none' }}>
        <table className="tbl">
          ...
          <tbody>
            {MASTER.hot_deals.map((d,i) => (
              <tr key={i} className="clickable">
                <td>{d.name}</td>
                <td className="muted">{d.sector}</td>
                <td className="muted">{CITIES.find(c=>c.id===d.city)?.name}</td>
...
function MasterFeed() {
  return (
    <div className="card">
      ...
      <div className="col" style={{ gap: 0 }}>
        {MASTER.notifications.map((n,i) => (
          <div key={i} className="row" style={{ padding: '12px 0', borderBottom: i < MASTER.notifications.length - 1 ? '1px solid var(--border-1)' : 0, gap: 12 }}>
